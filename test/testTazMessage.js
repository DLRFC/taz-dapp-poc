...
const { GROUP_ID, TAZMESSAGE_CONTRACT, SEMAPHORE_CONTRACT, IDENTITY_SEED } = require("../config/goerli.json")
...
describe("TazMessage", function () {
  ...
  before(async () => {
    ...
    if (DEPLOY_NEW_TAZ_MESSAGE_CONTRACT) {
      ...
      contract = await run("deployTazMessage", { semaphoreContract: SEMAPHORE_CONTRACT, logs: true })
  ...
  describe("# addMember", () => {
    ...
    it("Should fail to add a member if the member has already been added", async () => {
      ...
      const identityCommitment = identity.generateCommitment()
      const tx = contract.connect(signer1).addMember(GROUP_ID, identityCommitment)
      await expect(tx).to.be.revertedWith("Member has already been added to this group")
    })
  })
  ...
  describe("# addMessage", () => {
    it("Should add a message", async () => {    
      ...
      const proofElements = await run("createProof", { identitySeed: IDENTITY_SEED, groupId: GROUP_ID, signal: signal, logs: false })
  ...
  describe("# replyToMessage", () => {
    it("Should reply to a message", async () => {
      ...
      const tx = contract.connect(signer1).replyToMessage(
        parentMessageId,
        messageId,
        messageContent,
        proofElements.groupId,
     ...
     it("Should fail to reply to message when parentMessageId is empty", async () => {
      ...
      await expect(tx).to.be.revertedWith("Invalid ID provided for parent message")
    })
  })
})
