...
const blobForServingString = new Blob([stringPrefix + b64Data], {type: "text/plain"}) // Use for serving a string
...
const dataFileArrayForServingString = [new File([blobForServingString], "image.txt")]

web3StorageClient.put(dataFileArrayForServingImage, { wrapWithDirectory: false, }).then(
    (dataCid) => {
...
web3StorageClient.put(dataFileArrayForServingString, { wrapWithDirectory: false, }).then(
    (dataCid) => {
        const imagelUri = "https://" + dataCid + ".ipfs.dweb.link"
